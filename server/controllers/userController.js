...
export default class UserController {
    ...
    static async GetAllContactList(req, res) {
        try {
            const contactlist = await Contact.find();
            return res.status(200).json({ status: true, data: contactlist });
        } catch (error) {
            return res.status(500).json({ status: false, message: "Error fetching contact list", error });
        }
    }
